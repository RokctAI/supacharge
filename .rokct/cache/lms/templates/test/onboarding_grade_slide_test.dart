...
void main() {
  ...
  testWidgets('the whole IntroDeps slide list is student-gated', (tester) async {
    ...
    // The temporary admin shortcut must skip every student capture step —
    // it records nothing and lands straight on the done card.
    final admin = IntroNotifier(slides: slides)
      ..chooseRole(OnboardingRole.admin);
    expect(admin.currentSlide, isNull);
    expect(admin.hostSlidesComplete, isTrue);
    admin.dispose();
  });
}
