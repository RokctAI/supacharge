# Supacharge — Feature Guide

_Live tutoring that fits around school_

> This guide is generated automatically on every merge: CI builds the
> app with its built-in demo dataset, walks the guided tour on an
> Android emulator, and captures each screen below. Content shown is
> demo data.

## 1. Welcome to Supacharge

![Welcome to Supacharge](screenshots/01-welcome.png)

Supacharge is live tutoring that fits around school - and in this demo build, any sign-in details work.

## 2. Make it yours

![Make it yours](screenshots/02-ui_type.png)

Pick the look that suits how you study.

## 3. Today, sorted

![Today, sorted](screenshots/03-schedule.png)

Your day at a glance - the next live session is always front and centre.

## 4. Courses built on CAPS

![Courses built on CAPS](screenshots/04-courses.png)

Browse CAPS-aligned courses for every subject you take, term by term.

## 5. Meet the tutors

![Meet the tutors](screenshots/05-tutors.png)

Real tutors teaching live - find the one whose style clicks with you.

## 6. The library keeps everything

![The library keeps everything](screenshots/06-library.png)

Rewatch past lessons and dip into knowledge bites whenever you like.

## 7. Practice makes marks

![Practice makes marks](screenshots/07-practice.png)

Sharpen your skills with practice built around your subjects.

## 8. Climb the league

![Climb the league](screenshots/08-league.png)

Earn points as you learn and see how you stack up this week.

## 9. One clear plan

![One clear plan](screenshots/09-my_plan.png)

See exactly what your plan includes - no surprises.

## 10. Your learning, your profile

![Your learning, your profile](screenshots/10-profile.png)

Your Supacharge profile tracks your grade, subjects and progress in one place.
